...
class Wrapper:
    ...
    def add_robot_point_to_code(self, x_coor, y_coor, z_coor, spin=0):
        """ Create palletizing robot coordinate for the point in JSON format. """
        point_dict = {
            'x': x_coor,
            'y': y_coor,
            'z': z_coor,
        ...
        self.pathCodeList.append(point_dict)
        self.pointNo += 1

    def print_all(self):
        """ Generate a complete Python script with the Cartesian coordinates. """
...
"""
        ...
        for point in self.pathCodeList:
            python_script += f"            {repr(point)},\n"
...
"""
        ...
        print("End of file, write into %s.py ... " % self.filename)

        if not os.path.isdir('out'):
            os.mkdir('out')
        with open('out/%s.py' % self.filename, 'w') as write_file:
            write_file.write(python_script)
